...
async def upload_document(
    ...
    # Process PDF for RAG in background
    ...
    # Create an initial system message to show upload started
    try:
        system_message = models.Message(
            session_id=session_id,
            user_id=None,
            content=f"📎 **{current_user.name}** uploaded: **{file.filename}** (processing...)",
            role="system"
        )
        db.add(system_message)
        db.commit()
        
        # Broadcast the "processing" message
        from app.services.websocket_manager import manager
        from fastapi.encoders import jsonable_encoder
        msg_json = jsonable_encoder(schemas.MessageResponse.model_validate(system_message))
        background_tasks.add_task(manager.broadcast_to_session, session_id, {"type": "new_message", "message": msg_json})
    except Exception as e:
        print(f"Error creating initial system message: {e}")
